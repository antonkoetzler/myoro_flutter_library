...
void main() {
  ...
  blocTest(
    ...
    build: () => MyoroResolverBloc<dynamic>(() => throw const HttpException(validationErrorMessage)),
    act: (MyoroResolverBloc bloc) => bloc.add(const ExecuteRequestEvent()),
    expect: () => const [
      MyoroResolverState(status: MyoroRequestEnum.loading),
      MyoroResolverState(status: MyoroRequestEnum.error, errorMessage: validationErrorMessage),
    ],
  );
  ...
  blocTest(
    ...
    expect: () => const [
      MyoroResolverState(status: MyoroRequestEnum.loading),
      MyoroResolverState(status: MyoroRequestEnum.error, errorMessage: 'Exception: $validationErrorMessage'),
    ],
  );
  ...
  blocTest(
    ...
    act: (MyoroResolverBloc<String> bloc) => bloc.add(const ExecuteRequestEvent()),
    expect: () => const [
      MyoroResolverState<String>(status: MyoroRequestEnum.loading),
      MyoroResolverState<String>(status: MyoroRequestEnum.success, result: result),
    ],
  );
}
