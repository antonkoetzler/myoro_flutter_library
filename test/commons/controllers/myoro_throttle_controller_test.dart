...
void main() {
  ...
  test('MyoroThrottleController.run', () async {
    ...
    throttler.run(incrementValue);
    expect(value,
        1); // Did not run the callback as [throttler] is throttling callbacks.
    await Future.delayed(Duration(
        milliseconds: duration.inMilliseconds +
            2)); // Not too sure why we need to wait an extra 2 milliseconds...
    throttler.run(incrementValue);
    expect(value,
        2); // After waiting for the [throttler]'s duration, the callback will execute again.
  });
}
