...
void main() {
  ...
  final icon = kMyoroTestIcons[faker.randomGenerator.integer(kMyoroTestIcons.length)];
  final iconSize = faker.randomGenerator.boolean() ? faker.randomGenerator.integer(500).toDouble() : null;
  final text = faker.lorem.word();
  late final TextStyle? textStyle;
  final textAlign = faker.randomGenerator.boolean() ? TextAlign.values[faker.randomGenerator.integer(TextAlign.values.length)] : null;
  final padding = faker.randomGenerator.boolean() ? EdgeInsets.all(faker.randomGenerator.decimal()) : null;
  final mainAxisAlignment = faker.randomGenerator.boolean() ? MainAxisAlignment.values[faker.randomGenerator.integer(MainAxisAlignment.values.length)] : null;
  ...
  testWidgets('MyoroIconTextHoverButton', (WidgetTester tester) async {
      ...
      MyoroWidgetTester(
        child: Builder(
          builder: (BuildContext context) {
            themeExtension = context.resolveThemeExtension<MyoroIconTextHoverButtonThemeExtension>();
    ...
    expect(
      find.byWidgetPredicate(
        (Widget w) => w is Padding && w.padding == (padding ?? themeExtension.contentPadding) && w.child is Row,
      ),
      findsOneWidget,
    );
    ...
    expect(
      find.byWidgetPredicate(
        (Widget w) =>
            w is Row &&
            w.mainAxisAlignment == (mainAxisAlignment ?? themeExtension.mainAxisAlignment) &&
            w.children.length == 3 &&
            ...
            ((w.children.last as Expanded).child as Text).textAlign == (textAlign ?? TextAlign.left) &&
            ((w.children.last as Expanded).child as Text).maxLines == themeExtension.textMaxLines &&
            ((w.children.last as Expanded).child as Text).overflow == themeExtension.textOverflow,
      ),
      findsOneWidget,
    );
