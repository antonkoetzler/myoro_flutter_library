...
void main() {
  testWidgets('MyoroIconTextButtonWidgetShowcase', (WidgetTester tester) async {
    ...
    expect(find.byType(MyoroIconTextButton), findsNWidgets(8));
    expect(find.widgetWithText(WidgetShowcaseOption, 'Invert icon and text.'), findsOneWidget);
    expect(
      find.widgetWithText(WidgetShowcaseOption, 'Spacing between icon and text.'),
      findsOneWidget,
    );
    expect(find.byType(PaddingWidgetShowcaseOption), findsOneWidget);
    expect(
      find.widgetWithText(MyoroTapStatusEnumWidgetShowcaseOption, 'Content color builder'),
      findsOneWidget,
    );
    expect(find.byType(IconWidgetShowcaseOption), findsOneWidget);
    expect(find.widgetWithText(WidgetShowcaseOption, 'Icon size'), findsOneWidget);
    expect(find.widgetWithText(WidgetShowcaseOption, 'Text'), findsOneWidget);
    expect(find.widgetWithText(WidgetShowcaseOption, 'Text max lines'), findsOneWidget);
