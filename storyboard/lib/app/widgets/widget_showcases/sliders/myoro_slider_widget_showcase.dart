...
final class MyoroSliderWidgetShowcase extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => MyoroSliderWidgetShowcaseBloc(
        ...
        Positioned(
          child: ValueListenableBuilder(
            ...
            builder: (_, double width, __) {
              return Container(
                width: width,
                color: context.resolveThemeExtension<MyoroSliderWidgetShowcaseThemeExtension>().hiddenKittyContainerColor,
              );
            },
          ),
        ),
...
final class _Slider extends StatelessWidget {
  ...
  String _currentValueIndicatorBuilder(double value) => value.toStringAsFixed(0);
  String _maxValueIndicatorTextBuilder(double maxValue) => maxValue.toStringAsFixed(0);
    ...
    return BlocBuilder<MyoroSliderWidgetShowcaseBloc, MyoroSliderWidgetShowcaseState>(
      builder: (_, MyoroSliderWidgetShowcaseState state) {
        return MyoroSlider(
          ...
          width: state.width,
          currentValueIndicatorTextBuilder: state.currentValueIndicatorTextBuilderEnabled ? _currentValueIndicatorBuilder : null,
          maxValueIndicatorTextBuilder: state.maxValueIndicatorTextBuilderEnabled ? _maxValueIndicatorTextBuilder : null,
          footerIndicatorTextBuilder: state.footerIndicatorTextBuilderEnabled ? (double value) => _footerIndicatorTextBuilder(widgetSize, value) : null,
...
final class _LabelOption extends StatelessWidget {
  ...
  void _onChanged(BuildContext context, String text) {
    context.resolveBloc<MyoroSliderWidgetShowcaseBloc>().add(SetLabelEvent(text));
  }
...
final class _LabelTextStyleOption extends StatelessWidget {
  ...
  MyoroMenuItem _itemBuilder(TextStyle textStyle) {
    return MyoroMenuItem(
      text: MyoroTypographyTheme.instance.getTextStyleName(textStyle),
    );
  }
